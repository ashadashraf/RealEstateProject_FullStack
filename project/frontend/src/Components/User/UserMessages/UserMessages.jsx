...
class Chat extends Component {
  ...
  updateHistory = () => {
    const newPath = `/room/${this.props.propertyId}`;
                        ...
                        {/^schedule (\d{1,2}\/\d{1,2}\/\d{4}, \d{1,2}:\d{2}:\d{2} (AM|PM)) expire (\d{1,2}\/\d{1,2}\/\d{4}, \d{1,2}:\d{2}:\d{2} (AM|PM))$/i.test(message.content) ? (
                          <div class={`max-w-sm pl-6 pr-6 pt-3 pb-3 bg-gray-800 border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 ${this.isMessageExpired(message.content) ? 'cursor-not-allowed opacity-80' : ''}`}>
                            ...
                            {!this.isMessageExpired(message.content) && (
                              <a onClick={this.updateHistory} class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 cursor-pointer">
                                Connect
